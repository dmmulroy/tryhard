...
import { Result, Ok, Err, AnyResult } from "./result";
  ...
  describe("all", () => {
    describe("eager", () => {
      describe("default mode", () => {
        ...
        it("rejects with a panic when a promise rejects", async () => {
          const promise = Result.all([new Promise(() => {}), Promise.reject("boom")]);
          expect(promise).rejects.toBeInstanceOf(Panic);
          expect(promise).rejects.toHaveProperty("cause", "boom");
        });
      });
    ...
    describe("lazy", () => {
      describe("default mode", () => {
        it("returns result in order and adheres to concurrency", async () => {
          ...
          const [one, two, three, four] = [makeFn(1), makeFn(2), makeFn(3), makeFn(4)];

          const promise = Result.all([one.fn, two.fn, three.fn, four.fn], {
            concurrency: 2,
          });
        ...
        it("short circuits on first error", async () => {
          ...
          const promise = Result.all(
            [one.fn, two.fn, () => Promise.resolve(Result.err(3 as const)), four.fn],
            {
              concurrency: 2,
            },
          );
        ...
        it("rejects with a panic when a promise rejects", async () => {
          const promise = Result.all(
            [() => new Promise(() => {}) as any, () => Promise.reject("boom")],
            {
              concurrency: 2,
            },
          );
      ...
      describe("settled mode", () => {
        it("returns results in order and adheres to concurrency", async () => {
          ...
          expect(results).toEqual([Result.ok(1), Result.err(2), Result.ok(3), Result.ok(4)]);
        });
        it("rejects with a panic when a promise rejects", async () => {
          const promise = Result.all(
            [() => new Promise(() => {}) as any, () => Promise.reject("boom")],
